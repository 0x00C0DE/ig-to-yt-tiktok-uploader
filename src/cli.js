...
import { loadConfig, requireAccount } from "./config.js";
...
Setup a stored login:
  npm run setup -- --platform tiktok --account tt-main
...
async function setup(cwd, options) {
  ...
  const context = await openAccountBrowser({
    ...
    chromeProfileDirectory: config.defaults.chromeProfileDirectory
  });
...
function transferOptions(config, options) {
  ...
  return { instagramId, mode, destinations };
}

async function transferOne({ cwd, config, reelUrl, instagramId, mode, destinations, state, bridge }) {
  ...
  if (instagramId === "public" || config.defaults.instagramDiscoveryMethod === "extension") {
    try {
      source = await probeReel({ cwd, reelUrl, downloader: config.downloader });
    } catch (probeError) {
    ...
    const sourceContext = await openAccountBrowser({ cwd, platform: "instagram", accountId: instagramId, headless: config.defaults.headless });
    try { source = await readReelMetadata(sourceContext.pages()[0] || await sourceContext.newPage(), reelUrl); }
    finally { await sourceContext.close(); }
  }
  const metadata = mapMetadata(source);
  const videoPath = await downloadReel({ cwd, reelUrl, outputDirectory: config.defaults.downloadDirectory, downloader: config.downloader });
...
async function transfer(cwd, options) {
  ...
  const bridge = config.defaults.uploadMethod === "extension" ? new ExtensionBridge() : null;
...
async function sync(cwd, options) {
  ...
  try {
    if (bridge && config.defaults.instagramDiscoveryMethod === "extension") {
      console.log(`Requesting Reel discovery for @${String(options.handle).replace(/^@/, "")} in the current Chrome session...`);
      ...
      const context = selected.instagramId === "public"
        ? await openPublicBrowser({ headless: true })
        : await openAccountBrowser({ cwd, platform: "instagram", accountId: selected.instagramId, headless: config.defaults.headless });
      try {
